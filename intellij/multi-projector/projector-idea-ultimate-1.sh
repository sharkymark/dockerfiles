...
# Create duplicate versions of this file with different file names and port numbers
# to run multiple versions of the JetBrains IDE

IFS="$(printf '\n\t')"
# shellcheck disable=SC2086
"$JAVA_BIN" \
  -classpath "$CLASSPATH:/var/tmp/coder/projector-multi-editor-plugin/jetbrains/deps/projector-server/projector-server/build/distributions/lib/*" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_IDEA_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_IDEA.hprof" \
  -Didea.paths.selector=IdeaIIU2021.2.3 \
  "-Djb.vmOptionsFile=$VM_OPTIONS_FILE" \
  ${IDE_PROPERTIES_PROPERTY} \
  -Dorg.jetbrains.projector.server.port=8997 \
...
#  ${IDE_PROPERTIES_PROPERTY} \
#  -Djava.system.class.loader=com.intellij.util.lang.PathClassLoader -Didea.vendor.name=JetBrains -Didea.paths.selector=IntelliJIdea2021.3 -Didea.jre.check=true -Dsplash=true \
